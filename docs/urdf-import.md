...
| `tests/fixtures/unitree_go2/go2_minimal.urdf` | Unitree Go2 (12 R, meshless) | **Experimental** mammal quad tree FK / stance smoke. Not insectoid `Family=legged` Walk. Base-rooted (upstream has no floating joint). BSD-3 from `go2_description`. |
...
### Experimental quadruped (Go2)

`unitree_go2/go2_minimal.urdf` is a **CI fixture** for mammal sagittal 3R/leg FK — **not** Motus Walk / `LeggedLayout.QuadSmoke` (insectoid). Use `LoadTree` + Tree FK; optional `ExtractSerialTip("base", "FL_foot")`. Do **not** claim Bretl–Lall locomotion or Spot equivalence. Upstream [`go2_description`](https://github.com/unitreerobotics/unitree_ros/tree/master/robots/go2_description) already roots at `base` (no float→fixed rewrite).
