...
Motus.NET package identity targets **2.0.0** (NuGet publish on `v2.0.0` tag). Empty `TouchBodies` fails closed (`ArgumentException`). Until **2.0.0** is on nuget.org, build GH with `-UseLocal` / `-p:UseMotusNetProjectReference=true` against sibling Motus.NET tip. Motus.Core **1.8.0** is the last confirmed live NuGet while **1.9.0** / **2.0.0** index may lag.
