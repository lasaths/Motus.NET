...
|-----------|-------------------------------------------------------------------|
| Libraries on [nuget.org](https://www.nuget.org/profiles/lasaths) (tree **2.0.0**; NuGet publish on `v2.0.0` tag — **1.9.0** may still be indexing) | Rhino/GH UI that calls these APIs |
...
Changelog: [CHANGELOG.md](CHANGELOG.md). Package/`Directory.Build.props` targets **2.0.0** (NuGet publish on `v2.0.0` tag). GitHub [v1.9.0](https://github.com/lasaths/Motus.NET/releases/tag/v1.9.0) tagged; Motus.Core **1.8.0** is the last confirmed nuget.org index entry while **1.9.0** indexes. Use sibling project references until **2.0.0** is tagged and published.
