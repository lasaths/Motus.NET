...
|-----------|-------------------------------------------------------------------|
| Libraries on [nuget.org](https://www.nuget.org/profiles/lasaths) (latest published **0.17.0**) | Rhino/GH UI that calls these APIs |
...
Changelog: [CHANGELOG.md](CHANGELOG.md). Package/`Directory.Build.props` version is **0.17.0** on nuget.org ([GitHub v0.17.0](https://github.com/lasaths/Motus.NET/releases/tag/v0.17.0)). Master may be ahead of the pin (HolonomicSE3 / catalog fixtures) — use sibling project references for close-open-dev against unreleased tip; NuGet is the default consumer path.
